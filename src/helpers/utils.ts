...
export namespace utils {
  ...
  export function genApiUrl(
    path: string,
    queryArgs: object,
    underlizeKeys: boolean = false
  ): string {
    if (!queryArgs) {
      queryArgs = {};
    }
    if (underlizeKeys) {
      queryArgs = utils.underlize(queryArgs);
    }
    let args: string[] = [];
  ...
  export function parseTime(
    ...
    // ts定义中Date的构造器不支持number，但是事实上是支持的，因此此处使用了any
    let date = new Date(time as any);
  ...
  export function retryUntilResolve<T>(
    ...
    let res: RetryUntilResolveRes<T> = {
      handleMaxRetry: function(
        handler: MaxRetryHandler
      ): RetryUntilResolveRes<T> {
  ...
  export function numberToFixed(
    ...
    if (f.length < fixedLen && forceFiexed) {
      for (let i = 1; i <= fixedLen - f.length; i++) {
        res += "0";
      }
    }
    if (res[res.length - 1] === ".") {
  ...
  /** 合并两个object，会返回类型正确的对象 */
  export function merge<X extends {}, Y extends {}>(x: X, y: Y) {
